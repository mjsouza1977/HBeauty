...
unit Controller.Formata.HBeauty;
...
begin
    ...
    begin
        ...
        // Celular...
        if formato = erCelular then
            texto := Mask('(##)#####-####', SomenteNumero(texto));

        // CNPJ...
        if formato = erCNPJ then
            texto := Mask('##.###.###/####-##', SomenteNumero(texto));

        // CPF...
        if formato = erCPF then
            texto := Mask('###.###.###-##', SomenteNumero(texto));

        // Inscricao Estadual (IE)...
        if formato = erInscricaoEstadual then
            texto := FormataIE(SomenteNumero(texto), Extra);

        // CNPJ ou CPF...
        if formato = erCNPJouCPF then
        ...
        // CEP...
        if Formato = erCEP then
            texto := Mask('##.###-###', SomenteNumero(texto));

        // Data...
        if formato = erData then
            texto := FormataData(SomenteNumero(texto));

        // Peso...
        if Formato = erPeso then
